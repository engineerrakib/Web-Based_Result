		...
		<table id="example" class="table table-hover table-bordered text-center" style="width:100%">
		  <thead class="table-warning">
			...
			<tr>
			  ...
			  <th scope="col"><?php echo $row['s2'];?></th>
			  <th scope="col"><?php echo $row['s3'];?></th>
			  <th scope="col"><?php echo $row['s4'];?></th>
			  <th scope="col"><?php echo $row['s5'];?></th>
		  ...
		  <tbody>
				<?php
				$i=1;
				$x=mysqli_query($al,"SELECT * FROM marks WHERE sem='$sem' AND dept='$dept'");
				while($row=mysqli_fetch_array($x))
				{
					?>
			  <tr>
				...
				<td>
				  <a href="update_marks.php?updatemarks=<?php echo $row["id"];?>" class="link-dark" onclick="return confirm('Are You Sure..?');"><i class="fa-solid fa-pen-to-square fs-5 me-3"></i></a>
				  <a href="delete_marks.php?del-marks=<?php echo $row["id"];?>" class="link-dark" onclick="return confirm('Are You Sure..?');"><i class="fa-solid fa-trash fs-5"></i></a>
				</td>
			  </tr>
			<?php
			}
