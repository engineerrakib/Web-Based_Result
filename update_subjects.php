...
if (isset($_POST["submit"])) {
  ...
  $u =mysqli_query($al,"UPDATE subjects SET s1='$s1',s2='$s2',s3='$s3',s4='$s4',s5='$s5' WHERE id=$id");
...
<body>
  ...
  <div class="container">
	...
	<?php
    $sql =mysqli_query($al,"SELECT * FROM subjects WHERE id = $id LIMIT 1");
    ...
    <div class="container d-flex justify-content-center">
      <form action="" method="post" style="width:50vw; min-width:300px;">
		...
		<div class="mb-3">
          ...
          <input type="text" class="form-control" name="s2" value="<?php echo $row['s2'] ?>">
        </div>
		<div class="mb-3">
          <label class="form-label">Subject-3:</label>
          <input type="text" class="form-control" name="s3" value="<?php echo $row['s3'] ?>">
        </div>
		<div class="mb-3">
          <label class="form-label">Subject-4:</label>
          <input type="text" class="form-control" name="s4" value="<?php echo $row['s4'] ?>">
        </div>
		<div class="mb-3">
          <label class="form-label">Subject-5:</label>
          <input type="text" class="form-control" name="s5" value="<?php echo $row['s5'] ?>">
        </div>

        <div>
          <button type="submit" class="btn btn-success" name="submit">Update</button>
          <a href="viewSubjects.php" class="btn btn-danger">Cancel</a>
        </div>
		<br/>
		<br/>
		<br/>
